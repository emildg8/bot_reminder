...
from scripts.set_bot_avatar import DEFAULT_AVATAR, set_bot_avatar, verify_bot_has_avatar
...
def test_token_validation() -> None:
    from scripts.set_bot_avatar import is_plausible_bot_token

    assert not is_plausible_bot_token("")
    assert not is_plausible_bot_token("bad")
    assert is_plausible_bot_token("123456789:AAHabcdefghijklmnopqrstuvwxyz123456")
