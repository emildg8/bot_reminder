...
python scripts/smoke_nlp.py   # NLP smoke (локально / на сервере после pull)
...
- [ ] `/ping` → актуальная версия (**v3.43.0**)
- [ ] `/sysinfo` → STT: Groq (без Whisper local), deploy sha
- [ ] `python scripts/smoke_nlp.py` → `smoke_nlp OK`
