...
```bash
make verify
# verify_ops = файлы + версии + число тестов в docs + smoke_nlp + smoke_group_mentions + smoke_stars
...
- [ ] `/ping` → актуальная версия (**v3.45.2**)
- [ ] `/sysinfo` → STT: Groq (без Whisper local), deploy sha
- [ ] `python scripts/verify_ops.py` → `verify_ops OK · v3.45.2` (включает 3 smoke)
