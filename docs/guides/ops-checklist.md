...
**Проект v1.0:** ✅ готов — [PROJECT-COMPLETE.md](../PROJECT-COMPLETE.md).  
Ниже — **эксплуатация** при деплое / смене сервера, не backlog разработки.

## Проверка репозитория (локально / CI)

```bash
make verify
# verify_ops = файлы + версии + число тестов в docs + smoke_nlp + smoke_group_mentions + smoke_user_group_assignee + smoke_stars + smoke_author
...
- [x] Group Privacy **Turn off** — нужен для `@бот` в группах *(подтверждено prod)*
- [ ] Команды бота актуальны (обновляются при старте)

## Проверка после деплоя

- [x] `/ping` → **v3.46.4** на prod *(assignee #35)*
- [x] `/ping` в группе → `Group Privacy: выкл`
- [ ] `/sysinfo` → STT: Groq (без Whisper local), deploy sha (admin-mode)
- [ ] `python scripts/verify_ops.py` → `verify_ops OK · v3.46.4` (5 smoke)
...
### Assignee v3.46 — ✅ ФИНАЛ (2026-06-03, #35)

- [x] `@break_remind_bot Emil Через 1 минуту тест` → reply `👌 👤 Emil · …`
- [x] Confirm в личке → assignee Emil / `@emildg8`
- [x] Срабатывание `⏰ Emil, тест`
- [x] Офлайн: `make smoke-user-group` · `verify_ops`
...
- [ ] «Своя сумма» → число → confirm → invoice в личку
- [ ] «Своя сумма» → фраза с буквами → напоминание (не Stars)
