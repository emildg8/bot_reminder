...
**Статус:** ✅ v3.46.0 · код: `mention_create.py`, `mention_parse.py`, `assignee_prompt.py`, `create_confirm.py`, `collective_confirm.py`
...
| 2a | @бот + **имя из списка** + фраза | `@бот Emil через час` (тап по участнику в @) |
...
| 4 | **Голос** + reply / подпись `@бот` | ответ на бота или voice с caption `@бот` (@user в caption) |
...
- **@ из списка Telegram** (тап по имени) — в UI может быть **имя без `@`**, бот понимает через `text_mention`.
- Имя **набранное руками** без выбора из @ — бот предупредит, assignee может не найтись.
...
| В группе (до confirm) | `👌 👤 Emil · через 1 мин · тест` (**reply**) → подтверди в личке |
